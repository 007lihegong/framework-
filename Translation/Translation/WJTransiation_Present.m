...
#import "WJTransiation_Present.h"

@interface WJTransiation_Present ()

@property (nonatomic, assign) WJTransiation_PresentType type;

@end

@implementation WJTransiation_Present
...
- (NSTimeInterval)transitionDuration:(id<UIViewControllerContextTransitioning>)transitionContext{
    return _type == WJTransiation_PresentTypePresent ? 0.5 : 0.25;
}
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    switch (_type) {
        case WJTransiation_PresentTypePresent:
            [self presentAnimation:transitionContext];
            break;
            
        case WJTransiation_PresentTypeDismiss:
...
- (void)presentAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    toVC.view.frame = CGRectMake(0, CGRectGetHeight(containerView.bounds), CGRectGetWidth(containerView.bounds), CGRectGetHeight(containerView.bounds));//高400
    ...
    [UIView animateWithDuration:[self transitionDuration:transitionContext] delay:0 usingSpringWithDamping:0.55 initialSpringVelocity:1.0 / 0.55 options:0 animations:^{
        //首先我们让vc2向上移动
        toVC.view.transform = CGAffineTransformMakeTranslation(0, -CGRectGetHeight(containerView.bounds));//向上400
        //然后让截图视图缩小一点即可
